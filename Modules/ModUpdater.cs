...
public class ModUpdater
{
    ...
    //private static readonly string URL_Github = "https://api.github.com/repos/Loonie-Toons/TOHEBR-Restored";
    ...
    public static async Task<bool> CheckReleaseFromGithub(bool beta = false)
    {
        ...
        try
        {
            ...
            {
                client.DefaultRequestHeaders.Add("User-Agent", "TOHEBR Updater");
            ...
            else
            {
                ...
                {
                    if (assets[i]["name"].ToString() == "TOHEBR_Steam.dll" && Constants.GetPlatformType() == Platforms.StandaloneSteamPC)
                    {
                        downloadUrl = assets[i]["browser_download_url"].ToString();
                        break;
                    }
                    if (assets[i]["name"].ToString() == "TOHEBR_Epic.dll" && Constants.GetPlatformType() == Platforms.StandaloneEpicPC)
                    {
                        downloadUrl = assets[i]["browser_download_url"].ToString();
                        break;
                    }
                    if (assets[i]["name"].ToString() == "TOHEBR.dll")
                        downloadUrl = assets[i]["browser_download_url"].ToString();
                }
    ...
    public static bool NewVersionCheck()
    {
        try
        {
            ...
            if (Directory.Exists("TOH_DATA") && File.Exists(@"./TOHEBR-DATA/BanWords.txt"))
            {
    ...
    public static void DeleteOldFiles()
    {
        try
        {
            foreach (var path in Directory.EnumerateFiles(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "*.*"))
            {
                ...
                if (path.EndsWith("TOHEBR.dll")) continue;
    ...
    public static async Task<bool> DownloadDLL(string url)
    {
        try
        {
            var savePath = "BepInEx/plugins/TOHEBR.dll.temp";
...
#nullable disable
            ...
            else
            {
                ...
                File.Move("BepInEx/plugins/TOHEBR.dll.temp", fileName);
