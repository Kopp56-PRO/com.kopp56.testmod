...
using TOHEBR.Roles.AddOns.Crewmate;
using TOHEBR.Roles.AddOns.Impostor;
...
public static class Utils
{
    ...
    public static void ApplySuffix(PlayerControl player)
    {
        ...
        else
        {
            ...
            if (player.AmOwner && player.FriendCode != "gnuedaphic#7196" && player.FriendCode != "loonietoons" && player.FriendCode != "dovebliss#9271")
            {
                if (!player.IsModClient()) return;
                {
                    if (GameStates.IsOnlineGame || GameStates.IsLocalGame)
                        name = $"<color={GetString("HostColor")}>{GetString("HostText")}</color><color={GetString("NameColor")}>{name}</color>";
            ...
            else if (player.AmOwner)
            {
                ...
                {
                    SuffixModes.TOHEBR => name += $"\r\n<color={Main.ModColor}>TOHEBR-R v{Main.PluginDisplayVersion}</color>",
    ...
    {
        string f = $"{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}/TOHEBR-logs/";
        string t = DateTime.Now.ToString("yyyy-MM-dd_HH.mm.ss");
        string filename = $"{f}TOHEBR-v{Main.PluginVersion}-{t}.log";
        if (!Directory.Exists(f)) Directory.CreateDirectory(f);
        FileInfo file = new(@$"{Environment.CurrentDirectory}/BepInEx/LogOutput.log");
        file.CopyTo(@filename);
        if (PlayerControl.LocalPlayer != null)
            HudManager.Instance?.Chat?.AddChat(PlayerControl.LocalPlayer, string.Format(GetString("Message.DumpfileSaved"), $"TOHEBR - v{Main.PluginVersion}-{t}.log"));
